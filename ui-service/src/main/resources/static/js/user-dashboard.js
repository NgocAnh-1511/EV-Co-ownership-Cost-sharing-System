...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (path.includes('/user/costs')) {
        ...
        page = 'fair-schedule';
    } else if (path.includes('/user/maintenance')) {
        page = 'maintenance';
...
function switchPage(page) {
    ...
    switch(page) {
        ...
        case 'disputes':
            ...
            break;
        case 'maintenance':
            // handled by user-maintenance.js
            break;
    }
}
...
async function removeMember(groupId, memberId, userId) {
    ...
    try {
        ...
        const response = await authenticatedRequest(`${API.GROUPS}/${groupId}/members/${memberId}?currentUserId=${CURRENT_USER_ID}`, {
            method: 'DELETE'
        });
...
async function loadFundCategories() {
    try {
        const response = await authenticatedRequest(`${API.FUND}/transactions?userId=${CURRENT_USER_ID}`);
        if (!response.ok) throw new Error('Failed to load transactions');
