(function() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        setupSignaturePad();
        bindUserControls();
        loadUserVehicles().then(() => loadUserFairness());
    });
                        ...
                        if (refreshResponse.ok) {
                            ...
                            if (retryRes.ok) {
                                ...
                                vehicles.forEach(vehicle => {
                                    ...
                                    option.value = id;
                                    option.textContent = `${vehicle.vehicleName || vehicle.name} (${vehicle.ownershipPercentage || 0}%)`;
    ...
    async function loadUserReservations() {
        if (!currentVehicleId) {
            document.getElementById('userReservationSelect').innerHTML = '<option value="">Chưa chọn xe</option>';
            return;
        }
        try {
            const client = typeof window.authenticatedFetch === 'function'
            ...
            const upcoming = reservations
                .filter(r => r.user && Number(r.user.userId || r.userId) === Number(currentUserId))
                .filter(r => new Date(r.startDatetime || r.start).getTime() >= Date.now() - 3600000)
            ...
            upcoming.forEach(r => {
                ...
                option.textContent = `${new Date(r.startDatetime || r.start).toLocaleString('vi-VN')} (${r.status})`;
                select.appendChild(option);
            });
        } catch (error) {
