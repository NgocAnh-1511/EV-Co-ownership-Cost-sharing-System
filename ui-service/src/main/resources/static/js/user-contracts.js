(function () {
    ...
    function renderActionButtons(contract) {
        ...
        // Nếu đã có hợp đồng, hiển thị nút Chi tiết
        if (contract.hasContract && contract.contractId) {
            actions.push(`<button class="btn-ghost" data-action="view" data-id="${contract.contractId}">
                <i class="fas fa-eye"></i> Chi tiết
            </button>`);
        }

        // Xử lý nút ký/tải PDF
        ...
        } else if (contract.status === 'signed' || contract.status === 'archived') {
            // Đã ký: hiển thị nút tải PDF
            actions.push(`<button class="btn-outline" data-action="download" data-id="${contract.contractId}">
    ...
    function handleTableAction(event) {
        ...
        switch (button.dataset.action) {
            ...
            case 'sign':
                ...
                break;
            case 'download':
                notify('Tính năng tải PDF sẽ sớm có mặt.', 'info');
                break;
            case 'join':
