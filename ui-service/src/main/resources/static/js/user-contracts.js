...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Nắm bắt các phần tử filter từ HTML (nếu có)
    ...
    // Sửa lỗi: Đảm bảo chọn đúng nút bằng ID
    const createContractButton = document.getElementById('btn-create-contract');
    ...
    async function loadContracts() {
        try {
            ...
            // Tạm thời vô hiệu hóa các hàm filter này nếu các select không tồn tại
    ...
    function populateVehicleFilter(data) {
        if (!vehicleFilterSelect) return; // Kiểm tra nếu filter tồn tại
    ...
    function updateDashboard(data) {
        const statusFilter = statusFilterSelect ? statusFilterSelect.value : null;
        const vehicleFilter = vehicleFilterSelect ? vehicleFilterSelect.value : null;
    ...
    if (createContractButton) createContractButton.addEventListener('click', openCreateContractModal);
    if (modalCloseButton) modalCloseButton.addEventListener('click', closeCreateContractModal);
    if (modalCancelButton) modalCancelButton.addEventListener('click', closeCreateContractModal);
    if (createContractForm) createContractForm.addEventListener('submit', handleCreateContractSubmit);

    // Gắn sự kiện filter (nếu tồn tại)
