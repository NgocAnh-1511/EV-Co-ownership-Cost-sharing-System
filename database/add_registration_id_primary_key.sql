-- Script để thêm cột registration_id AUTO_INCREMENT làm primary key
...
-- ============================================
-- BƯỚC 4: XÓA PRIMARY KEY CŨ (COMPOSITE KEY)
-- ============================================
SELECT '=== XÓA PRIMARY KEY CŨ ===' AS Info;
ALTER TABLE vehicleservice DROP PRIMARY KEY;

-- ============================================
-- BƯỚC 5: KIỂM TRA XEM CỘT registration_id ĐÃ TỒN TẠI CHƯA
-- ============================================
...
-- ============================================
-- BƯỚC 6: THÊM CỘT registration_id NẾU CHƯA TỒN TẠI
-- ============================================
SELECT '=== THÊM CỘT registration_id ===' AS Info;
SET @sql = IF(@col_exists > 0, 
    'SELECT "Cột registration_id đã tồn tại, sẽ cập nhật..." AS Message', 
    'ALTER TABLE vehicleservice ADD COLUMN registration_id INT NOT NULL AUTO_INCREMENT FIRST');
...
SET @sql = IF(@col_exists > 0,
    'ALTER TABLE vehicleservice MODIFY COLUMN registration_id INT NOT NULL AUTO_INCREMENT',
    'SELECT "Cột registration_id đã được tạo mới với AUTO_INCREMENT" AS Message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- ============================================
-- BƯỚC 7: SET AUTO_INCREMENT START VALUE
-- ============================================
...
-- ============================================
-- BƯỚC 8: TẠO PRIMARY KEY MỚI (registration_id)
-- ============================================
SELECT '=== TẠO PRIMARY KEY MỚI (registration_id) ===' AS Info;
ALTER TABLE vehicleservice ADD PRIMARY KEY (registration_id);

-- ============================================
-- BƯỚC 9: TẠO INDEX CHO (service_id, vehicle_id) ĐỂ TỐI ƯU QUERY
-- ============================================
...
-- ============================================
-- BƯỚC 10: TẠO UNIQUE CONSTRAINT CHO (service_id, vehicle_id, status) 
-- CHỈ KHI status KHÔNG PHẢI 'completed'
-- ============================================
-- Note: MySQL không hỗ trợ partial unique index trực tiếp
-- Thay vào đó, chúng ta sẽ kiểm tra duplicate ở application layer
...
SELECT '✅ HOÀN TẤT! Đã thêm cột registration_id làm primary key' AS Result;
