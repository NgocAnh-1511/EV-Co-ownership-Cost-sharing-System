...
public class GroupManagementController {
    ...
    public ResponseEntity<?> addGroupMember(
        ...
        try {
            ...
            // New member: enforce contract signature requirement (skip for self-join, UI đã kiểm tra)
            if (!isSelfJoin) {
            Optional<GroupContract> latestContractOpt = groupContractRepository.findTopByGroup_GroupIdOrderByCreationDateDesc(groupId);
            if (latestContractOpt.isEmpty()) {
                logger.warn("⚠️ [GroupManagementController] Cannot add member {} to group {}: no contract found", groupMember.getUserId(), groupId);
                return ResponseEntity.status(409).body(Map.of(
                        "error", "missing_contract",
                        "message", "Nhóm chưa có hợp đồng. Vui lòng tạo và ký hợp đồng trước khi thêm thành viên."
                ));
            }

            GroupContract activeContract = latestContractOpt.get();
    ...
    public ResponseEntity<?> signContract(@PathVariable Integer contractId,
        ...
        try {
            ...
            if (alreadySigned) {
                Map<String, Object> membershipInfo = ensureMembershipAfterContractSignature(contract, userId);
                return ResponseEntity.ok(Map.of(
                        "success", true,
                        "alreadySigned", true,
    ...
    /**
     * Ensure that a user becomes a group member right after signing the contract.
     * The first member of the group will automatically become the Admin.
     */
    private Map<String, Object> ensureMembershipAfterContractSignature(GroupContract contract, Integer userId) {
        Map<String, Object> membershipInfo = new HashMap<>();
        Group group = contract.getGroup();
        Integer groupId = group.getGroupId();

        List<GroupMember> members = groupMemberRepository.findByGroup_GroupId(groupId);
        Optional<GroupMember> existingMemberOpt = members.stream()
                .filter(member -> member.getUserId().equals(userId))
                .findFirst();

        if (existingMemberOpt.isPresent()) {
            GroupMember existingMember = existingMemberOpt.get();
            membershipInfo.put("joined", true);
            membershipInfo.put("alreadyMember", true);
            membershipInfo.put("memberId", existingMember.getMemberId());
            membershipInfo.put("role", existingMember.getRole().name());
            membershipInfo.put("ownershipPercent", existingMember.getOwnershipPercent());
            membershipInfo.put("joinedAt", existingMember.getJoinedAt());
            membershipInfo.put("groupId", groupId);
            membershipInfo.put("isFirstMember", false);
            membershipInfo.put("isAdminAssigned", false);
            logger.info("ℹ️ [GroupManagementController] User {} is already a member of group {}. Skipping auto-join.", userId, groupId);
            updateGroupAdminByOwnership(groupId);
        ...
        newMember.setOwnershipPercent(0.0);
        newMember.setRole(firstMember ? GroupMember.MemberRole.Admin : GroupMember.MemberRole.Member);

        GroupMember savedMember = groupMemberRepository.save(newMember);
        logger.info("✅ [GroupManagementController] Auto-joined user {} to group {} via contract signature. MemberId={}, role={}",
                userId, groupId, savedMember.getMemberId(), savedMember.getRole());

        if (firstMember) {
            Integer previousAdminId = group.getAdminId();
            group.setAdminId(userId);
            groupRepository.save(group);
            logger.info("👑 [GroupManagementController] User {} set as Admin for group {} (previous admin: {}).",
                    userId, groupId, previousAdminId);
        }

        membershipInfo.put("joined", true);
        membershipInfo.put("alreadyMember", false);
        membershipInfo.put("memberId", savedMember.getMemberId());
        membershipInfo.put("role", savedMember.getRole().name());
        membershipInfo.put("ownershipPercent", savedMember.getOwnershipPercent());
        membershipInfo.put("joinedAt", savedMember.getJoinedAt());
        membershipInfo.put("groupId", groupId);
        membershipInfo.put("isFirstMember", firstMember);
        membershipInfo.put("isAdminAssigned", firstMember);
        updateGroupAdminByOwnership(groupId);
        return membershipInfo;
    }
