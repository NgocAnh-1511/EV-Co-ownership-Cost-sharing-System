...
import com.example.groupmanagement.dto.GroupMaintenanceOptionDto;
...
public class GroupManagementController {
    ...
    public ResponseEntity<?> getMaintenanceOptions(@PathVariable Integer userId) {
        try {
            logger.info("🔧 Fetching maintenance options for userId={}", userId);
            List<GroupMember> memberships = groupMemberRepository.findByUserId(userId);
            List<GroupMaintenanceOptionDto> options = memberships.stream()
                    .map(member -> {
                        Group group = member.getGroup();
                        Integer vehicleId = group.getVehicleId();
                        return GroupMaintenanceOptionDto.builder()
                                .groupId(group.getGroupId())
                                .groupName(group.getGroupName())
                                .adminId(group.getAdminId())
                                .vehicleId(vehicleId)
                                .vehicleLabel(vehicleId != null ? "Xe #" + vehicleId : null)
                                .memberId(member.getMemberId())
                                .memberRole(member.getRole() != null ? member.getRole().name() : null)
                                .ownershipPercent(member.getOwnershipPercent())
                                .build();
                    })
                    .collect(Collectors.toList());
            return ResponseEntity.ok(options);
        } catch (Exception e) {
            logger.error("❌ Failed to fetch maintenance options for userId={}", userId, e);
            return ResponseEntity.status(500)
                    .body(Map.of("error", "Failed to fetch maintenance options: " + e.getMessage()));
        }
    }
